...
Future table_command_tests(conn) {
  ...
  group("table commands:", () {
    ...
    test("must create table", () {
      _test(r.tableCreate("test_table",{"primaryKey": "testPk", "cacheSize" : 1024}), (response) => expect(response.created, isTrue));
    });
    ...
    test("must add data", () {
      _test(r.table('test_table').insert([{'test key':'test_val'}]), (response) => expect(response.inserted, isTrue));
    });
    test("must count data", () {
      _test(r.table('test_table').count(), (response) => expect(response, 1));
    });
    test("must create index", () {
      _test(r.table("test_table").indexCreate("myindex"), (response) => expect(response.created, isTrue));
    });
    ...
    test("must drop index", () {
      _test(r.table("test_table").indexDrop("myindex"), (response) => expect(response.dropped, isTrue));
    });

    test("must drop table", () {
      _test(r.tableDrop("test_table"), (response) => expect(response.dropped, isTrue));
    });
